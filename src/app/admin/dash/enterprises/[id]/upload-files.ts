...
export async function uploadFiles(
  eid: string,
  path: string,
  files: DropzoneFields[] | undefined,
  defaultFiles?: ({ extension: string } & Omit<DropzoneFields, 'file'>)[],
) {
    ...
    ? (files || []).reduce(
        (acc, item) => {
          ...
          if (defaultFilesId.includes(item.id) && Object.keys(fileModified).length)
            acc[0].push({ ...fileModified, id: item.id } as DropzoneFields);
        ...
        [[], []] as [DropzoneFields[], string[]],
      )
    : [[...(files || [])], []];

  if (filesRemoved.length) {
    await supabase.storage
      .from(`enterprises/${eid}`)
      .remove(filesRemoved.map((id) => `${path}/${id}`));
